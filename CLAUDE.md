...
# Run tests (233 tests, should complete in <1s)
.venv/bin/pytest
...
tests/
  test_config.py     # 80 tests — env var parsing, validation, defaults, scoping
  test_client.py     # 43 tests — API methods, errors, auth, delay
  test_cleaner.py    # 103 tests — staleness logic, short-circuit, fail-safe, dry-run, scoping
...
- The same rule applies to the **container**, and it is a separate check. `client.py` returns `data.get("groups", [])`, whose default fires only when the key is *absent* — a present-but-`null` value returns `None` and iterating it raises. So each of the four loops type-checks its listing (`isinstance(..., list)`) next to its `except APIError` block, with the same consequence: count an error, skip that subtree, mark the listing incomplete. `_extract_timestamp` guards the whole payload for the same reason — `_get` returns whatever the body decoded to, so a bare `null` or `[]` reaches it. What this costs if missed is worse than a skipped entry: a package already permanently deleted, every later group never walked (so its stale packages survive *every* future cycle), no `Cycle complete` summary line, and exit code 0 telling the scheduler it worked.
